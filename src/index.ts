...
import { GroupSchema, TemplateSchema, type Group, type Template } from "./models"
...
if (!(!!connectionString && !!bucketName && !!username && !!password)) {
    throw new Error("Missing Couchbase connection string, bucket name, username, and/or password")
}
...
export const Templates = ottoman.model<Template, Template>("templates", TemplateSchema, {
    modelKey: "type",
    idKey: "_id",
})
